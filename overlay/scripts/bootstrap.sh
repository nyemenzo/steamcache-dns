...
echo "Bootstrapping DNS from https://github.com/nyemenzo/cache-domains"
...
curl -s -o services.json https://raw.githubusercontent.com/nyemenzo/cache-domains/master/cache_domains.json
...
cat services.json | jq -r '.cache_domains[] | .name, .domain_files[]' | while read L; do
  if ! echo ${L} | grep "\.txt" >/dev/null 2>&1 ; then
	...
	if [ "$CONTINUE" == "true" ]; then
      ...
      curl -s -o ${L} https://raw.githubusercontent.com/nyemenzo/cache-domains/master/${L}
